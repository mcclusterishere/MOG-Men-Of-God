...
## Material-aware animation architecture
...
- Talent: rim catches light while the core glyph remains readable
...
For richer material response, use WebGL/canvas only where it earns its complexity. A small shader can combine base texture, roughness/specular proxy, and semantic mask so light sweeps affect only intended material regions.
